    ...
    if request.method == "POST":
        ...
        if password1 == password2:
            if not User.objects.filter(username=username,email=email).exists():
                ...
                messages.info(request,"Hey, This email or username is taken!")
                return redirect("signup")
        else:
            messages.info(request,"Hey Passwords doesn't match!")
            return redirect("signup")
    print("in signup")
    return render(request, "signup.html")
...
def login(request):
    if request.method == "POST":
        ...
        if user is None or user.email != email:
            ...
            # print(request.user.is_authenticated,"login yes")
            return redirect("/")
